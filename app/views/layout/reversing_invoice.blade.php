				    ...
				    <div class="box dark">
				        ...
				        <div id="div-1" class="body collapse in" aria-expanded="true">
    						<div class="alert alert-success">
    							<span class="pull-left"> إذن إرجاع أصناف </span>
    							<button class="btn btn-default btn-sm pull-right" style="margin-right: 5px;" onclick="printDiv('#add_reversing_invoice')">طباعة الفاتورة</button>
    							<button class="btn btn-primary btn-sm  pull-right printBarCodes">طباعة باركود للفاتورة</button>
    							<div style="clear:both;"></div>
    						</div>
				        	<div class="messages"></div>
				        	<div class="printLayout">
				        	{{Form::open(["action" => "add_reversing_invoice","id"=>"add_reversing_invoice" ,"class"=>"form-horizontal"])}}
					        	<div class="alert alert-info invoiceheader">
					        	 إذن إرجاع أصناف <span class="pull-right"> Myzoo Clinc </span></div>
					        	<div style="clear:both;"></div>
					        	<hr>
					        	<div class="row">
						        	<div class="col-xs-12">
						            		...
						            		<table class="itemsTable table table-bordered sortableTable responsive-table tablesorter tablesorter-default">
						            			<thead>
						            				<tr>
						            					...
						            					<th class="col-xs-2">سعر البيع للوحدة</th>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#add_new_items").on("click",function(){
			if ($(".item_code:last").val() != ""){
				...
				tableAppender += '</td>';
				tableAppender += '<td>';
				tableAppender += '<input id="item_price" name="item_price[]" placeholder="سعر البيع" class="form-control item_price" type="text" value="">';
				tableAppender += '</td>';
				tableAppender += '<td>';
				tableAppender += '<input id="item_total_price" name="item_total_price[]" placeholder="إجمالي سعر البيع" class="form-control item_total_price" type="text" value="">';
				tableAppender += '</td>';
				tableAppender += '</tr>';
				tableAppender += "<tr>";
                   ...
                   var newElement = ( /[\+]+/.test(elements[i]) ) ? elements[i].replace("+","_") : elements[i];
                    ...
                    $("input[type=text]").bind('keydown', elements[i], function assets() {
                        if (elements[i] == "Ctrl+s"){
                            $("#add_reversing_invoice").trigger("submit");
                        }
                        return false;
                   });
                });
	   ...
	   $(".date").datepicker({
		...
		$("#add_reversing_invoice").on("submit",function(){
			var formElementsData = new FormData(this);
			$.ajax({
				url : "add_reversing_invoice",
				dataType : 'json',
				type : 'post',
				contentType: false,
    			processData: false,
	...
	function getNextItemsOnStoresID(){
		return {{HomeController::getNextItemsOnStoresID('reversing_invoice')}}
	}
	...
	function getInvoiceData(invoiceID){
		...
		$.ajax({
			url : "getReversingInvoiceSingleItemsDataTable",
			dataType : 'json',
			type : 'get',
			data : {
				invoiceID : invoiceID,
			...
			success : function(data){
				if (data.errorsFounder == 1){
					...
					$.each(data.children,function(k,v){
						...
						tableAppender += '</td>';
						tableAppender += '<td>';
						tableAppender += '<input id="item_price" name="item_price[]" placeholder="سعر البيع" class="form-control item_price" type="text" value="'+v.item_price+'">';
						tableAppender += '</td>';
						tableAppender += '<td>';
						tableAppender += '<input id="item_total_price" name="item_total_price[]" placeholder="إجمالي سعر البيع" class="form-control item_total_price" type="text" value="'+v.item_total_price+'">';
						tableAppender += '</td>';
						tableAppender += '</tr>';
						tableAppender += "<tr>";
					});
	...
	function dataTableDrawer(){
		var sellInvoicedTabletbody = $(".sellInvoicedTable").dataTable({
			...
			"bServerSide": true,
			"bDestroy": true,
			"sAjaxSource": "getStoreReversingInvoiceItemsData",
			// "aaSorting": [[ 4, "desc" ]],
		});
